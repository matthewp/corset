...
QUnit.skip('can take an identifier as the second arg', assert => {
  let root = document.createElement('main');
  root.innerHTML = `<div id="app">Hello <span id="name"></span></div>`;
  let pet = { name: 'Wilbur' };
  let sheet = dsl`
...
QUnit.skip('can take a var as the first arg', assert => {
  let root = document.createElement('main');
  root.innerHTML = `<div id="app">Hello <span id="name"></span></div>`;
  let pet = { name: 'Wilbur' };
  let sheet = dsl`
...
QUnit.skip('if one arg, implicitly uses --scope as the context', assert => {
  let root = document.createElement('main');
  root.innerHTML = `<div id="app">Hello <span id="name"></span></div>`;
  let pet = { name: 'Wilbur' };
  let sheet = dsl`
